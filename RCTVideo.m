...
- (AVPlayerItem*)playerItemForSource:(NSDictionary *)source
{
    ...
    NSLog(@"VIDEO --URI-- is %@", uri   );
    if (isNetwork) {
        NSLog(@"VIDEO IS-NETWORK");
        //Load based on the URL
        url = [NSURL URLWithString:uri];
    }
    else if (isAsset) {
        NSLog(@"VIDEO IS-ASSET");
        ...
        NSError *error = nil;
        NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@".*/dvr/" options:NSRegularExpressionCaseInsensitive error:&error];
        NSString *newUri = [regex stringByReplacingMatchesInString:uri options:0 range:NSMakeRange(0, [uri length]) withTemplate:@"dvr/"];
        NSLog(@"NEW URI %@", newUri);
        NSString* localFile = [documentsPath stringByAppendingPathComponent:newUri];
        BOOL fileExists = [[NSFileManager defaultManager] fileExistsAtPath:localFile];
        if(fileExists)
        {
            NSLog(@"LOCAL EXISTS!");
            NSURL* localUrl = [[NSURL alloc] initFileURLWithPath:localFile isDirectory:NO];
            AVPlayerItem *localPlayerItem = [AVPlayerItem playerItemWithURL:localUrl];
    ...
    if (object == _playerItem) {
        ...
        if ([keyPath isEqualToString:statusKeyPath]) {
            ...
            if (_playerItem.status == AVPlayerItemStatusReadyToPlay) {
                NSLog(@"AVPlayer AVPlayerItemStatusReadyToPlay");
